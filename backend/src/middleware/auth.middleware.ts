...
export async function authenticate(req: Request, _res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    // Only enforce server-side session lookup when Postgres is available.
    // When running without Postgres (in-memory fallback), sessions are lost on
    // server restart — in that case trust the JWT signature alone so users
    // don't get logged out on every server restart during development.
